...
double wcda_ang(double ep)
{
  ...
  if(ep<=exb[0]) theta = angoptb[0];
  else if(ep>=exb[nvvb-1]) theta = angoptb[nvvb-1];
  else
  {
    while(ep>exb[i]) i++;
    theta = (angoptb[i]-angoptb[i-1])/(exb[i]-exb[i-1]) * (ep-exb[i-1]) + angoptb[i-1];
  }
  if(theta<0||isnan(theta)!=0) theta=0.;
  return theta;
}
...
int main(int argc,char *argv[])
{
  ...
  if(argc<3) { printf("%s  input(d.dat)  ediff  [reso1]  [reso2]  [t_reuse]  [1 for monitor mode]  (use log10(ediff))  set Reso1=0 for default\n",argv[0]);exit(0);}
  ediff=atof(argv[2]);
//  reso1=1.5852*wcda_ang(ediff);
  wcda_ang_maxsigma(&reso1,&maxsigma,&keepratio_ga,&keepratio_cr,&num_ga,&num_cr);
  if(argc>=4&&atof(argv[3])>1e-4)  reso1=atof(argv[3]);
  if(argc>=5)  reso2=atof(argv[4]);
  if(argc>=6)  t_reuse=atoi(argv[5]);
  if(argc==7)  ismon=atoi(argv[6]);
  //  reso=atof(argv[2]);
  strcpy(filename,argv[1]);
  if((fp_in=fopen(strcat(argv[1],"d.dat"),"rb"))==NULL) {printf("cannot open input file.\n"); exit(0);}
  ...
  for(t=0;t<t_reuse;t++)  {
    ...
    for(i=0;i<NRA;i++)      {
      if(ismon==1)  printf("%d/%d    initiating!   %3d%% \r",t,t_reuse,int(i*100/NRA));
      for(j=0;j<NDEC;j++)    	 { 
...
//printf("%f\n",beta[i][j]);
  ...
  if(ismon==1) printf("ROOT file part 1 is preparing \n");
...
//  TH2D *hnoff = new TH2D("hnon","N_off",NRA,0.,360.,NDEC,DEC_MIN,DEC_MAX);
  ...
  for (j=0;j<NDEC;j++)   {            // for reusing the selected grid when DEC is same.
    if(ismon==1) printf("point source smooth set!   %3d%%\r",int(j*100/NDEC));
    for (i=0;i<NRA;i++)        {
      ...
      if(isnan(signi[i][j])!=0||isinf(signi[i][j])!=0) signi[i][j]=0;
...
//printf("%d\n",smth_num);
  ...
  ///ADD LARGE SMOOTH//////////////////////////////////////////////////////
  if(ismon==1) printf("ROOT file part 2 is preparing \n");
  ...
  for (j=0;j<NDEC;j++)   {
    if(ismon==1) printf("large area smooth set!   %3d%%\r",int(j*100/NDEC));
    for (i=0;i<NRA;i++)        {
      sig_correct(i,j,reso2,equa_sys_width,&signi[i][j],&intens[i][j],&num_sig,&num_bg,&num_bin);
       ...
       if(fabs(intens[i][j]-1)/signi[i][j]<0.0002) h5s->Fill((i+0.5)*equa_sys_width,(j+0.5)*equa_sys_width+DEC_MIN,intens[i][j]);
    }
  }
  
  
  ///////////////mid structure////////////////////////////////////////////
  if(ismon==1) printf("ROOT file part 3 is preparing \n");
  ...
  for (i=0;i<NRA;i++)   {
    if(ismon==1) printf("ps - area smooth  set!   %3d%%\r",int(i*100/NRA));
    for (j=0;j<NDEC;j++)        {
  ...
  if(i0==0)
  {
    ...
    for(i=0;i<NRA;i++)
    {
      ...
      for(j=0;j<NDEC;j++)
      {
        ...
        if(r_smooth < distance_equatorial(ra,dec,ra0,dec0)) continue;
        smth[smth_num][0] = i;
        smth[smth_num][1] = j;
        smth_num++;
      }
