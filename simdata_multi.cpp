...
//  double       grid_sr;
  ...
  emid_ga=atof(argv[2]);
  if(atof(argv[3])>1e-4) emax_ga=atof(argv[3]);
  if(emid_ga>emax_ga)  {printf("emax_ga should be larger than emid_ga!\n");exit(0);}
  days=atof(argv[4]);
  if(argc>=6)  isroot=atoi(argv[5]);
  if(argc>=7)  ismon=atoi(argv[6]);

  strcpy(filename,argv[1]);
  if((fp_out=fopen(strcat(argv[1],".dat"),"wb+"))==NULL)    {printf("cannot create output file\n");exit(0);}
  strcpy(argv[1],filename);
  if((fp_log=fopen(strcat(argv[1],".log"),"w+"))==NULL)    {printf("cannot create log file\n");exit(0);}
  strcpy(argv[1],filename);
  if((fp_nb=fopen(strcat(argv[1],"_tmp_nb"),"wb+"))==NULL)    {printf("cannot create temp files\n");exit(0);}
  strcpy(argv[1],filename);
...
//printf("%f %f %f %d\n",thetamin,thetamax,area_nze[tmp_i],NAZ0[tmp_i]);
  ...
  if(ismon==1) printf("parameters fixed\n");
...
//printf("evnum_p[%d]= %f \n",i,evnum_p[i]);
  ...
  if(ismon==1) printf("background event numbers in grid has been calculated \n");
...
//exit(0);
    ...
    if(isroot==1)  {
      ...
      if(ismon==1) printf("ROOT file part2 is preparing\n");
...
double wcda_theta_dist(double theta)
{
  ...
  if(bin_num>700) theta_dist_density=0.;
...
//    n_cr = ((TH1F*)file_ang->Get("hpol_cr_smth"))->Integral(1,i);
    ...
    if(sigma[i-1]<n_tmp||n_cr<1e-5) continue;
    n_tmp = sigma[i-1];
    n_maxsigma = i;
  }
