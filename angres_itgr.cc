...
//  t1->SetBranchAddress("energy",&energy);
  ...
  {
    t1ga->GetEntry(i);
    if(nTop<nTop_le||nTop>=nTop_ue||thetaF<0||zenistar>60.|| nTop/(vcxPE[6]+0.01)<copt||dAngle>7.557) continue;
  ...
  {
    t1cr->GetEntry(i);
    if(nTop<nTop_le||nTop>=nTop_ue||thetaF<0||zenistar>60.|| nTop/(vcxPE[6]+0.01)<copt) continue;
